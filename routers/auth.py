...
@router.post('/test_admin')
def test_admin(current_user: str = Depends(oauth2.get_current_super_user)):
    return current_user


@router.post('/test_doctor')
def test_admin(current_user: str = Depends(oauth2.get_current_doctor)):
    return current_user
